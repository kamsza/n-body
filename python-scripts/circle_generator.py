...
if __name__ == '__main__':
    ...
    bodies = []
    for i in range(0, N):
        bodies.append([i, positions[i][0], positions[i][1], forces[i][0], forces[i][1], velocities[i][0], velocities[i][1]])

    bodies_str = '\n'.join([' '.join(map(str, body)) for body in bodies])

    with open("../src/main/resources/circle_10.txt", "w") as output_file:
